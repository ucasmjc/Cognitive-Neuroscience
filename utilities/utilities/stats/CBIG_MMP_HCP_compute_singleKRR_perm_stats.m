...
for h=1:N_seeds
	...
	load(cell2mat(fullfile(singleKRR_dir, seed, 'results', sub_fold_file.name)));
	...
	load(strjoin([fullfile(singleKRR_dir, seed, 'results'), ['\final_result_' outstem '.mat']],''));
	...
	if ~exist(outfile)
		for i = 1:N_fold
		    disp(['fold ' num2str(i)]);
		    % load data
		    load(strjoin([fullfile(singleKRR_dir, seed, 'results', 'y'), ['\fold_' num2str(i) '\y_regress_' outstem '.mat']],''));
